# coding: utf-8
 ...
 '''
 思路：二分查找的变形，注意到旋转数组的首元素肯定不小于旋转数组的尾元素，
 设置中间点。如果中间点大于首元素，说明最小数字在后面一半，如果中间点小于尾元素，
 说明最小数字在前一半。依次循环。同时，当一次循环中首元素小于尾元素，
 说明最小值就是首元素。但是当首元素等于尾元素等于中间值，只能在这个区域顺序查找。
 '''
...
class Solution:
    def minNumberInRotateArray(self, rotateArray):
        ...
        if rotateArray[front] < rotateArray[rear]:
            ...
            while rear - front > 1: # 三个以上元素的情况，使用二分查找
                mid = (front + rear)//2
...
Test = Solution()
print(Test.minNumberInRotateArray([2, 1]))
